...
output:
  ...
  pdf_document:
    fig_height: 6
    fig_width: 8
Author: Siu Kit Yeung
---

Github link: https://github.com/yskjdm/Experimental-Meta-Analysis-Template/tree/master 
...
Meta-analysis of the Omission-bias, as well as Fillon, Kutscher, and Feldman (2020) Impact of past behavior normality on regret: Meta-analysis of exceptionality effect. The corresponding datafile is "Experimental Meta-Analysis Template Full Coding Sheet.xlsx" (simulated data). Please ensure the updated version of this file is in the directory. Other collaborators include John Protzko, Adrien Fillon, Mahmoud Medhat Elsherif, and David Moreau. To export, please click "Knit" just under the filename in RStudio. You may export to a word document or a HTML document. Please ensure you have a custom.css file in the folder.
...
filename<-"Experimental Meta-Analysis Template Full Coding Sheet.xlsx"
...
if(!require(grDevices)){install.packages('grDevices', dependencies = TRUE)}
if(!require(sessioninfo)){install.packages('sessioninfo', dependencies = TRUE)}

#To install dmetar from github (currently under development)
if (!require("devtools")) {
  install.packages("devtools")
}
if (!require("dmetar")){
  install.packages("MathiasHarrer/dmetar")}
#If you fail to install, please check https://bookdown.org/MathiasHarrer/Doing_Meta_Analysis_in_R/dmetar.html
...
# Setting formatting options (Feel free to change based on your preference). The below code disables scientific notation. You may remove or keep it
...
# Removing the numbers from the column names
dat <- read_excel("Experimental Meta-Analysis Template Full Coding Sheet.xlsx", 
    sheet = "Included studies effect coding", skip=4, .name_repair = "universal")
dat$p.values=dat$p.values...87
dat$p.values.1=dat$p.values...110
dat$p.values.2=dat$p.values...137
dat$Calculated.Chisquare=dat$Calculated.Chisquare...119
dat$Reported.Chisquare=dat$Reported.Chisquare...118
dat$X..Cell.1=dat$..Cell.1
dat$X..Cell.2= dat$..Cell.2
dat$N.sample.size..post.attrition.=dat$N.sample.size...post.attrition.
dat$Are.cells.compared.to.each.other.or.to.a.constant.=dat$Are.cells.compared.to.each.other.or.to.a.constant....94   
dat$If.to.a.constant...what.constant.=dat$If.to.a.constant...what.constant....95
...
for (i in 1:nrow(dataset)) {
  ...
  if (!is.na(dataset$Cell.1.N[i]) & 
    ...
    if (!is.na(dataset$N.sample.size..post.attrition.[i]) & 
      ...
      # if we only have t statistic and N per each condition (within-subjects) - we use Lakens (2013) formula 7 for effect size caclulation
      if (!is.na(dataset$N.sample.size..post.attrition.[i]) &
          !is.na(dataset$t.from.t.test[i])& 
          (dataset$Design.type[i] == "Within-subject")) {
       
        tmp3 <- NA
        tmp3$es <- dataset$t.from.t.test[i] / sqrt(dataset$N.sample.size..post.attrition.[i])
        # print(tmp3)
        if (dataset$Does.effect.need.to.be.reversed.[i]== "Yes") {
          tmp3$es <- tmp3$es*(-1)
        }
        tmp3$var <- (1/dataset$N.sample.size..post.attrition.[i])+tmp3$es^2/(2*dataset$N.sample.size..post.attrition.[i])
        # print(tmp3)
        if (dataset$Does.effect.need.to.be.reversed.[i]== "Yes") {
          tmp3$es <- tmp3$es *(-1)
        }
        ...
        if (!is.na(dataset$Best.effect.estimate[i]) & 
          ...
          if (!is.na(dataset$Best.effect.estimate[i]) & 
            ...
            if (!is.na(dataset$Calculated.Chisquare[i]) & 
                !is.na(dataset$N.sample.size..post.attrition.[i])) {
              tmp5 <- NA
              tmp5 <- chies(dataset$Calculated.Chisquare[i], 
...
# Now we combine the effect sizes and variances with the dataset
...
Firstly, we start by running Random-Effects Two-Level Model, which is a commonly used model for meta-analyses, assuming different studies included have different true effects. You will find the effect size and confidence intervals, with heterogeneity measures including Q-statistics and I-square, which are essential statistics to be reported.
...
Afterwards, we run a Multivariate Three-Level Model, with the third level being Article Name. This takes account into the possible dependence between effect sizes of the same articles. We believe that this is a better approach for estimating effect size than two-level model. For details, see Cheung (2014) - https://pdfs.semanticscholar.org/8038/1b0d516d07e42e308f983624bffd497b1d9b.pdf . You will find the effect size and confidence intervals, with heterogeneity measures including Q-statistics and I-square, which are essential statistics to be reported.
...
A forest plot is a plot that visualizes the effect sizes, confidence intervals, and sample sizes of all the included studies.

```{r fig.width=16, fig.height=15}         

png(filename = "forestall.png",
    width = 2000, height = 1750, res = 180)

forest(allexpcollapsed, alim=c(-3.25,4), xlim=c(-4,5), ilab=totalexp$N.sample.size..post.attrition., ilab.xpos=-2.35, xlab = "Phenomenon name effect", cex=.8)
#Add headlines to the forest, with Bells and whistles
op <- par(font=4)
text(-4, allexpcollapsed$k+2, "Study",    pos=4, cex=.8)
text( 5, allexpcollapsed$k+2, "Observed [95% CI]",  pos=2, cex=.8)
text(-2.35, allexpcollapsed$k+2, "Sample size",cex=.8)
par(op)

dev.off()
```

![ForestPlot](./forestall.png)
...
[Describe findings from the funnel plot]
...
funnel(allexpcollapsed, level=c(95, 99), shade=c("white", "red"), refline=0)

png(filename = "vizfunnel.png",
    width = 2000, height = 1000, res = 200)
viz_funnel(allexpcollapsed, y_axis = "se", contours = TRUE, 
           trim_and_fill = TRUE, trim_and_fill_side = "left", text_size = 5, point_size = 4, 
           sig_contours = TRUE,  detail_level = 5,
           egger = TRUE, xlab = "Hedge's g")
dev.off()

```

![FunnelPlot](./funnel.png)
![FunnelPlotViz](./vizfunnel.png)
...
[Describe trim and fill findings]
```{r echo=FALSE, message=FALSE, warning=FALSE}
# Run trim and fill
...
[Describe Egger's regression test findings.]
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
[Describe Three-parameter selection model findings]
...
[Describe Henmi and Copas (2010) findings]
...
MetaForest is a package for addressing limited statistical power in moderator analyses with bootstrapping techniques. It provides a ranking of moderators in terms of variable importance. For more detail, please check https://cran.r-project.org/web/packages/metaforest/metaforest.pdf

[Describe MetaForest findings]
...
png(filename = "DV1.png",
    width = 2000, height = 1750, res = 150)

forest(expdvmDV1, alim=c(-5,2.85), xlim=c(-4,5), ilab=dataset2$N.sample.size..post.attrition., ilab.xpos=-3.50, xlab = "Phenomenon name effect", cex=.8)
op <- par(font=4)

text(-4.50, expdvmDV1$k+2, "Study",    pos=4, cex=.8)
text( 5, expdvmDV1$k+2, "Observed [95% CI]",  pos=2, cex=.8)
text(-3.50, expdvmDV1$k+2, "Sample size",                  cex=.8)
par(op)
...
png(filename = "DV2.png",
    width = 2000, height = 1750, res = 150)

forest(expdvDV2, alim=c(-3.25,4.25), xlim=c(-4,5), ilab=dataset2$N.sample.size..post.attrition., ilab.xpos=-2.35, xlab = "Phenomenon name effect", cex=.8)
op <- par(font=4)

text(-4, expdvDV2$k+2, "Study",    pos=4, cex=.8)
text( 5, expdvDV2$k+2, "Observed [95% CI]",  pos=2, cex=.8)
text(-2.35, expdvDV2$k+2, "Sample size",                  cex=.8)
par(op)
...
png(filename = "DV3.png",
    width = 2200, height = 1750, res = 155)

forest(expdvDV3, alim=c(-5,2.85), xlim=c(-4,5), ilab=dataset2$N.sample.size..post.attrition., ilab.xpos=-3.50, xlab = "Phenomenon name effect", cex=.8)
op <- par(font=4)

text(-4.50, expdvDV3$k+2, "Study",    pos=4, cex=.8)
text( 5, expdvDV3$k+2, "Observed [95% CI]",  pos=2, cex=.8)
text(-3.50, expdvDV3$k+2, "Sample size",                  cex=.8)
par(op)

dev.off()
```

![ForestPlotDV3](./DV3.png)

### Comparing DV types  ###

[Describe findings of different DVs, and compare different DV types.]
...
png(filename = "mod1cat1.png",
    width = 2200, height = 1750, res = 155)

forest(mod1cat1, alim=c(-5,4), xlim=c(-4,5), ilab=dataset2$N.sample.size..post.attrition., ilab.xpos=-3.50, xlab = "Phenonmenon name effect", cex=.8)
op <- par(font=4)

text(-4.5, mod1cat1$k+2, "Study",    pos=4, cex=.8)
text( 5, mod1cat1$k+2, "Observed [95% CI]",  pos=2, cex=.8)
text(-3.50, mod1cat1$k+2, "Sample size",                  cex=.8)
par(op)
...
png(filename = "mod1cat2.png",
    width = 2200, height = 1750, res = 155)

forest(mod1cat2, alim=c(-5,4.35), xlim=c(-4,5), ilab=dataset2$N.sample.size..post.attrition., ilab.xpos=-3.50, xlab = "Phenonmenon name effect", cex=.8)
op <- par(font=4)

text(-4.50, mod1cat2$k+2, "Study",    pos=4, cex=.8)
text( 5, mod1cat2$k+2, "Observed [95% CI]",  pos=2, cex=.8)
text(-3.50, mod1cat2$k+2, "Sample size",                  cex=.8)
par(op)
...
png(filename = "mod1cat3.png",
    width = 2200, height = 1750, res = 155)

forest(modcat3, alim=c(-3.25,4.25), xlim=c(-4,5), ilab=dataset2$N.sample.size..post.attrition., ilab.xpos=-2.35, xlab = "Phenonmenon name effect", cex=.8)
op <- par(font=4)

text(-4, modcat3$k+2, "Study",    pos=4, cex=.8)
text( 5, modcat3$k+2, "Observed [95% CI]",  pos=2, cex=.8)
text(-2.35, modcat3$k+2, "Sample size",                  cex=.8)
par(op)
...
png(filename = "mod2cat1.png",
    width = 2200, height = 1750, res = 155)

forest(expMod2Cat1, alim=c(-3.25,4.25), xlim=c(-4,5), ilab=dataset2$N.sample.size..post.attrition., ilab.xpos=-2.35, xlab = "Phenonmenon name effect", cex=.8)
op <- par(font=4)

text(-4, expMod2Cat1$k+2, "Study",    pos=4, cex=.8)
text( 5, expMod2Cat1$k+2, "Observed [95% CI]",  pos=2, cex=.8)
text(-2.35, expMod2Cat1$k+2, "Sample size",                  cex=.8)
par(op)
...
png(filename = "mod2cat2.png",
    width = 2200, height = 1750, res = 155)

forest(expMod2Cat2, alim=c(-3.25,5.95), xlim=c(-4,6.5), ilab=dataset2$N.sample.size..post.attrition., ilab.xpos=-2.35, xlab = "Phenonmenon name effect", cex=.8)
op <- par(font=4)

text(-4, expMod2Cat2$k+2, "Study",    pos=4, cex=.8)
text( 6.35, expMod2Cat2$k+2, "Observed [95% CI]",  pos=2, cex=.8)
text(-2.35, expMod2Cat2$k+2, "Sample size",                  cex=.8)
par(op)

dev.off()
```

![mod2cat2](./mod2cat2.png)
...
png(filename = "mod2cat3.png",
    width = 2200, height = 1750, res = 155)

forest(expMod2Cat3, alim=c(-4.7,3.5), xlim=c(-4,5), ilab=dataset2$N.sample.size..post.attrition., ilab.xpos=-3.45, xlab = "Phenonmenon name effect", cex=.8)
op <- par(font=4)

text(-4.7, expMod2Cat3$k+2, "Study",    pos=4, cex=.8)
text( 4.5, expMod2Cat3$k+2, "Observed [95% CI]",  pos=2, cex=.8)
text(-3.45, expMod2Cat3$k+2, "Sample size",                  cex=.8)
par(op)
...
png(filename = "mod3cat1.png",
    width = 2200, height = 1750, res = 155)

forest(expMod3Cat1, alim=c(-3.25,4.25), xlim=c(-4,5), ilab=dataset2$N.sample.size..post.attrition., ilab.xpos=-2.50, xlab = "Phenonmenon name effect", cex=.8)
op <- par(font=4)

text(-4, expMod3Cat1$k+2, "Study",    pos=4, cex=.8)
text( 5, expMod3Cat1$k+2, "Observed [95% CI]",  pos=2, cex=.8)
text(-2.50, expMod3Cat1$k+2, "Sample size",                  cex=.8)
par(op)

dev.off()
```

![mod3cat1](./mod3cat1.png)
...
png(filename = "mod3cat2.png",
    width = 2200, height = 1750, res = 155)

forest(expMod3Cat2, alim=c(-4.75,3.5), xlim=c(-4,5), ilab=dataset2$N.sample.size..post.attrition., ilab.xpos=-3.50, xlab = "Phenonmenon name effect", cex=.8)
op <- par(font=4)

text(-4.75, expMod3Cat2$k+2, "Study",    pos=4, cex=.8)
text( 5, expMod3Cat2$k+2, "Observed [95% CI]",  pos=2, cex=.8)
text(-3.50, expMod3Cat2$k+2, "Sample size",                  cex=.8)
par(op)
...
png(filename = "pub.png",
    width = 2200, height = 1750, res = 155)

forest(exppublished, alim=c(-3.25,4.25), xlim=c(-4,5), ilab=dataset2$N.sample.size..post.attrition., ilab.xpos=-2.5, xlab = "Phenonmenon name effect", cex=.8)
op <- par(font=4)

text(-4, exppublished$k+2, "Study",    pos=4, cex=.8)
text( 5, exppublished$k+2, "Observed [95% CI]",  pos=2, cex=.8)
text(-2.5, exppublished$k+2, "Sample size",                  cex=.8)
par(op)
...
png(filename = "unpub.png",
    width = 2200, height = 1750, res = 155)

forest(expunpublished, alim=c(-3.25,4.25), xlim=c(-4,5), ilab=dataset2$N.sample.size..post.attrition., ilab.xpos=-2, xlab = "Phenonmenon name effect", cex=.8)
op <- par(font=4)

text(-4, expunpublished$k+2, "Study",    pos=4, cex=.8)
text( 5, expunpublished$k+2, "Observed [95% CI]",  pos=2, cex=.8)
text(-2.35, expunpublished$k+2, "Sample size",                  cex=.8)
par(op)
...
png(filename = "mod4cat1.png",
    width = 2200, height = 1750, res = 155)

forest(mod4cat1, alim=c(-3.25,4.25), xlim=c(-4,5), ilab=dataset2$N.sample.size..post.attrition., ilab.xpos=-2.55, xlab = "Phenonmenon name effect", cex=.8)
op <- par(font=4)

text(-4, mod4cat1$k+2, "Study",    pos=4, cex=.8)
text( 5, mod4cat1$k+2, "Observed [95% CI]",  pos=2, cex=.8)
text(-2.55, mod4cat1$k+2, "Sample size",                  cex=.8)
par(op)

dev.off()
```

![Mod4Cat1](./mod4cat1.png)
...
png(filename = "mod4cat2.png",
    width = 2200, height = 1750, res = 135)

forest(mod4cat2, alim=c(-4.75,3.5), xlim=c(-4,5), ilab=dataset2$N.sample.size..post.attrition., ilab.xpos=-3.55, xlab = "Phenonmenon name effect", cex=.8)
op <- par(font=4)

text(-4.55, mod4cat2$k+2, "Study",    pos=4, cex=.8)
text( 5, mod4cat2$k+2, "Observed [95% CI]",  pos=2, cex=.8)
text(-3.55, mod4cat2$k+2, "Sample size",                  cex=.8)
par(op)
...
png(filename = "between.png",
    width = 2200, height = 1750, res = 155)

forest(modbetween, alim=c(-3.25,4.25), xlim=c(-4,5), ilab=dataset2$N.sample.size..post.attrition., ilab.xpos=-2.35, xlab = "Phenonmenon name effect", cex=.8)
op <- par(font=4)

text(-4, modbetween$k+2, "Study",    pos=4, cex=.8)
text( 5, modbetween$k+2, "Observed [95% CI]",  pos=2, cex=.8)
text(-2.35, modbetween$k+2, "Sample size",                  cex=.8)
par(op)
...
png(filename = "within.png",
    width = 2200, height = 1750, res = 155)

forest(modwithin, alim=c(-3.25,4.25), xlim=c(-4,5), ilab=dataset2$N.sample.size..post.attrition., ilab.xpos=-2.50, xlab = "Phenonmenon name effect", cex=.8)
op <- par(font=4)

text(-4, modwithin$k+2, "Study",    pos=4, cex=.8)
text( 5, modwithin$k+2, "Observed [95% CI]",  pos=2, cex=.8)
text(-2.50, modwithin$k+2, "Sample size",                  cex=.8)
par(op)
...
# write.csv(dataset, "Experimental-Studies-Meta-Analysis-Excel-Template-outfile.csv") (optional)
